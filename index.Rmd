...
# drop observations in EV dataset where base MSRP or driving range equal zero
EV <- EV %>%
  filter(base_msrp != 0,
         electric_range != 0,
         state == "WA")

# drop columns in zipcode dataset that are not latitude or longitude
zipcode <- zipcode %>%
  select(zip, lat, lng) %>%
  rename(zip_code = zip) %>%
  rename(long = lng) %>%
...
ggplot() +
  geom_polygon(washington_map, 
              ...
              fill = "white") +
  geom_point(cities,
             mapping = aes(x = long, y = lat, size = city_EV_count),
             alpha = 0.3,
             color = "blue") +
...
ggplot(EV, aes(x = base_msrp)) +
  geom_histogram(fill = "red3", bins = 40) +
  xlim(c(0, 200000)) +
...
The correlation coefficient of driving range and city EV count is -0.0486; on average, for every 1 mile increase in driving range, there is a 0.0486 decrease in the number of EVs in a city. The negative coefficient reveals a counterintuitive trend that disproves the hypothesis, implying that a higher driving range results is a lesser city EV count, albiet the effect size relatively small. Similar to base MSRP, this suggests that longer ranges are not necessarily a decisive factor for consumers (Graph 6, Table 1).
...
With more time and money, the analysis could be improved through an experimental study on a broader geographic area. In this study, I would also incorporate multivariable regressions to control for potential confounding factors. This would allow us to interpret conclusions causally and generalize the results to the public.
